...
class QuitPaginator(Dialog):
    ...
    async def run(
        ...
        try:
            await self.ctx.bot.wait_for('reaction_add', timeout=timeout, check=check)
...
class EmbedPaginator(Dialog):
    ...
    async def run(
        ...
        if len(self.pages) == 1:
            paginator = QuitPaginator(self.ctx, self.pages[0])
            await paginator.run(maybe_users, maybe_channel, timeout)
            return
        else:
        ...
        async def check_reactions():
            ...
            while True:
                ...
                if emoji == self.control_emojis[0]:
                    ...
                    return await self.quit(self.quit_text)
